...
class SimpleSignalExtractor:
    ...
    def extract(self, vla_output: Dict[str, torch.Tensor]) -> torch.Tensor:
        ...
        epistemic_uncertainty = actions.std(dim=1, unbiased=False).mean(dim=-1, keepdim=True)  # (B, 1)

        # 2. Action magnitude
        action_magnitude = mean_action.norm(dim=-1, keepdim=True)  # (B, 1)

        # 3. Action variance
        action_variance = actions.var(dim=1, unbiased=False).mean(dim=-1, keepdim=True)  # (B, 1)
        ...
        # 5. Max per-dimension variance
        per_dim_var = actions.var(dim=1, unbiased=False)  # (B, action_dim)
