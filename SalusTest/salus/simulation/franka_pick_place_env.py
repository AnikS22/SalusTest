...
class FrankaPickPlaceEnv:
    ...
    def convert_6d_to_7d_action(self, action_6d: torch.Tensor) -> torch.Tensor:
        ...
        Solution: Use first 6 joints from VLA, set 7th joint to a safe default.

        Args:
            action_6d: (num_envs, 6) action from VLA

        Returns:
            action_7d: (num_envs, 7) action for Franka
        """
        batch_size = action_6d.shape[0]
        action_7d = torch.zeros(batch_size, 7, device=action_6d.device, dtype=action_6d.dtype)

        # Copy first 6 joints
        action_7d[:, :6] = action_6d

        # Set 7th joint (panda_joint7 - wrist rotation) to neutral position
        # Default: 0.785 rad (45 degrees) - common neutral wrist angle
        action_7d[:, 6] = 0.785
    ...
    def step(self, actions: torch.Tensor):
        ...
        Args:
            actions: (num_envs, 6 or 7) joint position targets
                    6D: from VLA (will be converted to 7D)
                    7D: direct Franka control
